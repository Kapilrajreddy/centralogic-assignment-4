const Skills=()=>{
    ...
    const FrontEndSkills = [
      ...
      {
        name: "Express Js",
        image: "https://paths.tinkerhub.org/images/ExpressJS.jpg.png",
      },
      {
        name: "Python",
        image:
    ...
    return (
      <div className="mt-4 mb-4 border-b-[1px] font-titleFont border-b-black" id="skills">
        <h1 className="text-3xl md:text-4xl text-gray-300 font-bold capitalize mb-4">
          Skills
        </h1>
        {/* <h3 className="text-2xl md:text-3xl text-designColor font-bold capitalize">
          Front End
        </h3> */}
        <div className="flex flex-wrap gap-10 justify-center pb-8">
          {FrontEndSkills.map((each) => (
            <div className="bannerIcon xs:w-2/5 md:w-1/5 p-3 flex flex-col gap-2 md:h-40 xs:h-28">
              <div className="xs:w-10 xs:h-10 md:w-20 md:h-20 mb-1">
                <img src={each.image} alt="" className="w-full" />
              </div>
              <p className="font-medium md:text-xl xs:text-sm">{each.name}</p>
            </div>
          ))}
        </div>
      </div>
